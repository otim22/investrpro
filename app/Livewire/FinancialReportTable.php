...
final class FinancialReportTable extends PowerGridComponent
{
    ...
    public function addColumns(): PowerGridColumns
    {
        return PowerGrid::columns()
            ->addColumn('title', function (FinancialReport $model) { 
                return Str::limit($model->title, 40);
            })
            ->addColumn('description', function (FinancialReport $model) { 
    ...
    {
        $financialReport = FinancialReport::findOrFail($rowId);
        $this->redirectRoute('financial-reports.show', $financialReport);
    }

    public function actions(FinancialReport $row): array
    {
        return [
            Button::add('show')
                ->slot('View')
