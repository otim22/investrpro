...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12 col-12">
            <div class="d-flex justify-content-between">
                <div>
                    <h5 class="fw-bold py-1 text-capitalize"><span class="text-muted fw-light">Archive / <a href="{{ route('saved-emails.index') }}">Saved emails</a> / </span>{{ $savedEmail->title }}</h5>
                </div>
                <div>
                    <div class="btn-group" role="group">
                        ...
                        <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                            <a class="dropdown-item btn-sm" href="{{ route('saved-emails.edit', $savedEmail) }}">
                                <i class='me-2 bx bxs-edit-alt'></i>
                                Edit saved email
                            </a>
                            <a class="dropdown-item btn-sm" href="javascript:void(0);" data-bs-toggle="modal"
                                data-bs-target="#confirmSavedEmailDeletion{{ $savedEmail->id }}">
            ...
            <form action="{{ route('saved-emails.destroy', $savedEmail) }}" class="hidden" id="delete-saved-email-{{ $savedEmail->id }}"
                method="POST">
                @csrf
                @method('delete')
            </form>
            <div class="modal fade" id="confirmSavedEmailDeletion{{ $savedEmail->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="confirmSavedEmailDeletion{{ $savedEmail->id }}">
                                {{ $savedEmail->title }}</h5>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-primary"
                                onclick="event.preventDefault(); document.getElementById('delete-saved-email-{{ $savedEmail->id }}').submit();">Delete</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-xxl">
            <div class="card p-3">
                <div class="card-body">
                    <form method="POST" action="{{ route('saved-emails.update', $savedEmail) }}" enctype="multipart/form-data">
                            @csrf
                            @method('patch')
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-9">
                                <input 
                                    ...
                                    name="title"
                                    value="{{ old('title', $savedEmail->title) }}"
                                    placeholder="January" 
                                    autofocus
                                    disabled
                                />
                            ...
                            <div class="col-sm-9">
                                <div class="input-group input-group-merge">
                                    <textarea
                                        ...
                                        class="form-control"
                                        disabled
                                    >{{ old('description', $savedEmail->description) }} </textarea>
                                    @error('description')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="col-sm-9">
                                <div class="col-sm-9">
                                    @if ($savedEmail->getFirstMediaUrl('doc_attachement'))
                                        <div><a class="text-decoration-none" href="{{ route('saved-emails.download', $savedEmail->id) }}" target="_blank">{{ $savedEmail->getFirstMedia("doc_attachement")->name }}</a></div>
                                    @else
                                        <div>--</div>
                                    @endif
                                </div>
